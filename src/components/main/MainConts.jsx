...
const MainConts = () => {
    ...
    useEffect(() => {
        fetchAPI(
            `q=-19%26+love+소설+subject:fiction&printType=books&langRestrict=ko&orderBy=relevance&maxResults=32`
            ).then(
                (data) => setImages(data.items)
                )
                        ...
                        <main id="mainConts" className="main__container">
                            ...
                            <BookSearch page={page} startIndex={startIndex} setPage={setPage} setStartIndex={setStartIndex} />
                                ...
                                <span className="genre_pick">
                                    오늘은 <Link to=''></Link><em>추리 / 스릴러</em> 가 어떠신가요!
                                </span>
                            </div>
